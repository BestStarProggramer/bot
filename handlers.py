...
async def cancel_selection_handler(callback: CallbackQuery):
    """Полная отмена действия и удаление временного меню выбора."""
    u_id = callback.from_user.id
...
async def clear_selection_handler(callback: CallbackQuery):
    ...
    if u_id in user_selections:
        ...
        await callback.answer("Ошибка: сессия выбора не найдена", show_alert=True)
...
async def cmd_start(message: Message):
    await message.answer("🤖 Бот запущен!", reply_markup=get_keyboard(message.from_user.id))
...
async def cmd_swap_text(message: Message, command: CommandObject):
    ...
    await message.answer("🔄 Очередь обновлена", reply_markup=get_keyboard(message.from_user.id))