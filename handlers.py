...
def get_keyboard(user_id):
    if is_admin(user_id):
        buttons = [
            ...
            [InlineKeyboardButton(text="📋 Текущая очередь", callback_data="pub_queue")],
            ...
            [InlineKeyboardButton(text="📝 Список одногруппников", callback_data="pub_list"),
        ...
        buttons = [[InlineKeyboardButton(text="📝 Список одногруппников", callback_data="pub_list"),
...
async def confirm_swap_ui(callback: CallbackQuery):
    ...
    last_time = get_queue_time()
    text = f"🔄 <b>Очередь обновлена ({last_time}):</b>\n\n"
    for item in new_q:
...
async def handle_pub_btn(callback: CallbackQuery):
    ...
    elif callback.data == "pub_queue":
        await show_current_queue(callback, u_id)
...
async def show_current_queue(target, user_id):
    current_q = load_queue_from_db()
    last_time = get_queue_time() 
    
    if not current_q:
        text = "⚠️ Очередь еще не была сгенерирована."
    else:
        text = f"📋 <b>Текущая очередь</b> (от {last_time}):\n\n"
        for item in current_q:
            pos, _, name, is_p, is_l, _ = item
            pref = "⭐ " if is_p else "🐌 " if is_l else ""
            text += f"{pos}. {pref}{name}\n"
...
async def cmd_view_queue(message: Message):
    await show_current_queue(message, message.from_user.id)
